...
- [ ] **SECT-01**: **Database Schema Migration**: Alter the SQLite `assets` table to add a nullable `sector` TEXT column in a safe, repeatable migration.
- [ ] **SECT-02**: **Backend Update Service & API**: Expose a `PUT /api/assets/[ticker]/sector` endpoint to assign a sector tag to an asset.
...
- [ ] **CHK-01**: **Checklist Database Schema**: Create `asset_checklist` table to store status (yes/no/unsure) for each standard Barsi criterion per stock.
- [ ] **CHK-02**: **Backend Checklist APIs**: Create `GET /api/assets/[ticker]/checklist` and `PUT /api/assets/[ticker]/checklist` endpoints to retrieve and persist stock checklist answers.
